import React, { useState } from 'react';
import { Link, useNavigate } from 'react-router-dom';
import { useAuth } from '../lib/auth-context';
import { ApiError } from '../lib/api';
import { Button, Input, Alert } from '../design-system';
import { Logo } from '../components/Logo';

export const RegisterProducerPage: React.FC = () => {
  const { registerProducer } = useAuth();
  const navigate = useNavigate();
  const [form, setForm] = useState({ name: '', email: '', password: '', farmName: '', location: '' });
  const [error, setError] = useState('');
  const [isSubmitting, setIsSubmitting] = useState(false);

  const update = (field: keyof typeof form) => (e: React.ChangeEvent<HTMLInputElement>) =>
    setForm((f) => ({ ...f, [field]: e.target.value }));

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setError('');
    setIsSubmitting(true);
    try {
      await registerProducer(form);
      navigate('/producteur', { replace: true });
    } catch (err) {
      if (err instanceof ApiError && err.status === 409) {
        setError('يوجد حساب مسجل بهذا البريد الإلكتروني بالفعل.');
      } else {
        setError('تعذر إنشاء الحساب. تحقق من البيانات المدخلة.');
      }
    } finally {
      setIsSubmitting(false);
    }
  };

  return (
    <div dir="rtl" className="min-h-screen flex flex-col items-center justify-center bg-[#FAF6EE] px-4 py-10">
      <Link to="/" className="mb-8">
        <Logo compact />
      </Link>

      <div className="w-full max-w-sm bg-white rounded-2xl shadow-md border border-[#EAE1D2] p-6 sm:p-8">
        <h1 className="text-xl font-bold text-[#0C261B] mb-1 text-center">انضم كمنتج عسل</h1>
        <p className="text-sm text-gray-500 text-center mb-6">أنشئ حسابك لإرسال أول طلب تحقق من عسلك</p>

        {error && <Alert tone="error" className="mb-4">{error}</Alert>}

        <form onSubmit={handleSubmit} className="space-y-4">
          <Input label="الاسم الكامل" required value={form.name} onChange={update('name')} placeholder="أحمد بن صالح" />
          <Input
            label="البريد الإلكتروني"
            type="email"
            required
            value={form.email}
            onChange={update('email')}
            placeholder="example@email.com"
          />
          <Input
            label="كلمة المرور"
            type="password"
            required
            minLength={8}
            value={form.password}
            onChange={update('password')}
            hint="8 أحرف على الأقل"
          />
          <Input
            label="اسم المنحل / المزرعة"
            required
            value={form.farmName}
            onChange={update('farmName')}
            placeholder="منحل بن صالح"
          />
          <Input
            label="الموقع"
            required
            value={form.location}
            onChange={update('location')}
            placeholder="الكاف، تونس"
          />
          <Button type="submit" fullWidth isLoading={isSubmitting}>
            إنشاء الحساب
          </Button>
        </form>

        <p className="text-sm text-gray-500 text-center mt-6">
          لديك حساب بالفعل؟{' '}
          <Link to="/connexion" className="font-bold text-[#D49B37] hover:text-[#C68A28]">
            سجّل الدخول
          </Link>
        </p>
      </div>
    </div>
  );
};
