...
const TABS: { value: ProductStatut | 'ALL'; label: string }[] = [
  { value: 'ALL', label: 'الكل' },
  { value: 'BROUILLON', label: 'مسودة' },
  { value: 'PUBLIE', label: 'منشور' },
  { value: 'RUPTURE', label: 'نفدت الكمية' },
  { value: 'SUSPENDU', label: 'موقوف' },
];
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <h1 className="text-2xl font-bold text-[#0C261B]">المنتجات</h1>
      ...
      {!isLoading && filtered.length === 0 && (
        <Card>
          <EmptyState title="لا توجد منتجات في هذه الفئة" />
        </Card>
      )}
        ...
        {filtered.map((p) => (
          <Link key={p.id} to={`/admin/produits/${p.id}`}>
            <Card className="hover:border-[#D49B37] transition-colors h-full">
              ...
              <p className="text-sm font-bold text-[#D49B37]">{p.prix} د.ت</p>
              <p className="text-xs text-gray-400">المخزون: {p.stock}</p>
            </Card>
          </Link>
        ))}
      </div>

      <Modal isOpen={isOpen} onClose={() => setIsOpen(false)} title="منتج جديد" maxWidth="max-w-xl">
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <Input label="الفئة السعرية (اختياري)" placeholder="Premium" value={form.gamme} onChange={(e) => setForm((f) => ({ ...f, gamme: e.target.value }))} />
          <Select
            label="الدفعة المرتبطة (اختياري الآن، ضروري للنشر)"
            value={form.batchId}
          ...
          <Input
            label="روابط الصور (افصل بينها بفاصلة، اختياري)"
            value={form.images}
